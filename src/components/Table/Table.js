...
import { ChevronLeftIcon, ChevronRightIcon, ChevronDoubleRightIcon, ChevronDoubleLeftIcon, XMarkIcon, PencilIcon } from '@heroicons/react/24/outline';
...
export function IdDisplay({ value, onDetailsClicked }) {
	return (
		...
		<span
			className={'px-3 py-2 uppercase leading-wide font-bold text-md bg-gray-50 text-gray-600 rounded-full shadow-sm hover:cursor-pointer'}
	...
	return (
		<span
			className={classNames(
				...
				status?.startsWith('PENDING') || status?.startsWith('INPROCESS') || status?.startsWith('REGISTERED') || status?.startsWith('INDELIVERY') || status?.startsWith('INPROGRESS')
					? 'bg-yellow-100 text-yellow-700'
					: null,
				status.startsWith('unknown') || status.startsWith('CANCELED') ? 'bg-red-100 text-red-700' : null
			)}
		>
			{value}
		</span>
...
export function BasicPhoneNumber({ value }) {
	return (
		// <div className='flex items-center justify-center'>
		<span className={'px-3 py-2 uppercase leading-wide font-semibold text-primary text-md'}>+225 {value}</span>
		// </div>
	);
}
			...
			<div className='flex items-center cursor-pointer'>
				...
				<div className='ml-4'>
					...
					<div className={`text-sm text-base-`}>
						Cash Operation: <span className={`mx-2 ${operator?.is_cash ? 'text-error' : 'text-success'}`}>{operator?.is_cash ? 'YES' : 'NO'}</span>
					</div>
				</div>
			</div>
		</>
						...
						wallet?.wallet_type?.libelle !== 'CREDIT_SELLER' && (
							...
							>
								<span
									className={classNames(
										'px-3 py-1  uppercase leading-wide font-bold text-xs rounded-full shadow-sm',
										wallet?.wallet_type?.libelle.startsWith('MERCHANT') ? 'bg-violet-100 text-violet-700' : null,
										wallet?.wallet_type?.libelle.startsWith('PERSONAL') ? 'bg-blue-100 text-blue-700' : null,
										wallet?.wallet_type?.libelle.startsWith('LIVREUR') ? 'bg-teal-100 text-teal-700' : null
								...
								>
									{wallet?.wallet_type?.libelle + ' WALLET: '} <span className='font-semibold'>{(wallet?.balance || 0) + ' CFA'}</span>
								</span>
			...
			<div className=''>
				<div className='text-sm font-medium text-gray-900 my-2'>
					<span className={classNames(' py-1 uppercase leading-wide font-bold text-xs')}>
						{amount ? '' : 'Balance: '}
						<span className={'font-semibold ' + amount && !previousBonus ? 'text-primary' : ''}>{wallet ? wallet.balance : amount ? value : previousAmount || 0} CFA</span>
					</span>
					{!amount ? (
						<span className={classNames('px-3 py-1 uppercase leading-wide font-bold text-xs ')}>
							Bonus: <span className='font-semibold'>{wallet ? wallet.bonus : previousBonus || 0} CFA</span>
						</span>
					) : (
						<></>
					)}
					...
					(wallet) =>
						wallet?.wallet_type?.libelle !== 'CREDIT_SELLER' && (
							...
							>
								...
								>
									{wallet?.wallet_type?.libelle + ' WALLET: '} <span className='font-semibold'>{(wallet?.bonus || 0) + ' CFA'}</span>
								</span>
								{/*{wallet?.wallet_type?.libelle + " WALLET: "} {}*/}
							</div>
						)
	...
	return (
		...
		>
			<div className='flex-shrink-0 h-10 w-10'>
				<img
					className='h-10 w-10 rounded-full'
					src={row.original?.logo && row.original?.logo?.startsWith('http') ? row?.original?.logo : streetLogo}
					// src={row.original[column.imgAccessor]}
					alt=''
				/>
			</div>
	...
	return (
		...
		>
			<div className='flex-shrink-0 h-10 w-10'>
				<img
					className='h-10 w-10 rounded-full'
					src={row.original?.photo && row.original?.photo?.startsWith('http') ? row.original?.photo : streetLogo}
					// src={row.original[column.imgAccessor]}
					alt=''
				/>
			</div>
			<div className='ml-4'>
				...
				<div className={`text-sm  ${TYPE_OBJ['code'] === 'MARCH' ? 'text-primary' : 'text-gray-500'}`}>
					{MERCHANT_NAME ? cliTruncate(MERCHANT_NAME.toLocaleUpperCase(), 15) : TYPE_OBJ[column.clientTypeAccessor.split('.')[1]] || 'N/A'}
				</div>
			</div>
		</div>
	);
...
export function CashFlowCell({ value, column, row, phoneNumber }) {
	...
	const formattedSenderPhoneNumber = (senderPhoneNumber !== 'N/A' ? '+225 ' : '') + senderPhoneNumber;
		...
		<div className='flex items-center cursor-pointer'>
			<div className='ml-4'>
				<div className='text-sm text-gray-900 font-semibold'>
					...
					<span className='mx-3'>{isMobileMoneyTransaction ? 'MOBILE MONEY' : formattedSenderPhoneNumber ? formattedSenderPhoneNumber : 'N/A'}</span>
				</div>
				<div className='text-sm text-gray-900 font-semibold mt-1'>
					<div className='badge badge-xs badge-success'></div>
					<span className='mx-3'>{receiverPhoneNumber ? '+225 ' + receiverPhoneNumber : 'N/A'}</span>
				</div>
			</div>
		</div>
	);
...
export function AmountCell({ value, client, column, row }) {
	...
	return (
		<span className={classNames('px-3 py-1 uppercase leading-wide font-bold text-xs rounded-full shadow-sm', transactionFromClient ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700')}>
			{value} FCFA
		</span>
	);
}
...
export function Table({ columns, data, currPage, onLoad, updateFormValue, showFilter, firstActionButton }) {
