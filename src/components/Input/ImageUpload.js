...
const ImageUpload = (props) => {
  ...
  const [previewUrl, setPreviewUrl] = useState(
    props.defaultValue && props.defaultValue?.length
      ? props.defaultValue?.startsWith('http')
        ? props.defaultValue
        : `${process.env.REACT_APP_BASE_URL_DEV}/${props.defaultValue}`
      : ''
  );
  ...
  return (
    <>
      <div className="form-control mt-3">
        ...
        />
        <div
          className={`flex justify-center items-center flex-col image-upload ${
            props.center && 'center'
          }`}
        >
          ...
          {!props.disabled && (
            <button type="button" onClick={pickImageHandler} className="btn btn-outline btn-ghost">
              CHOOSE AN IMAGE
            </button>
          )}
        </div>
