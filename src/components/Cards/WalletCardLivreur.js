...
const WalletCardLivreur = ({ client, wallet, phone_number, onWalletClicked }) => {
	...
	useEffect(() => console.log(livreurs.find((liv) => liv?.id === client?.id)?.wallets?.find((w) => w?.id === wallet?.id)), []);
	...
	return (
		...
		>
			<img
				...
				src={livreurs.find((liv) => liv?.id === client?.id)?.wallets?.find((w) => w?.id === wallet?.id)?.wallet_type?.code !== 'MARCH' ? clientCard : merchantCard}
				alt=''
			/>
				...
				<div className='justify-between hidden sm:flex'>
					<div className=''>
						...
						<p className='font-medium tracking-widest'>
							{livreurs.find((liv) => liv?.id === client?.id)?.wallets?.find((w) => w?.id === wallet?.id)?.wallet_type?.libelle}
							<span
								className={`mx-2 inline-grid rounded-lg w-3 h-3 justify-items-center self-center items-center ${
									livreurs.find((liv) => liv?.id === client?.id)?.wallets?.find((w) => w?.id === wallet?.id)?.wallet_status?.code === 'ACTIVATED' ? 'bg-success' : 'bg-red-900'
								}`}
							></span>
						</p>
					</div>
				...
				<div className='pt-6 pr-6'>
					<div className='flex justify-between'>
						<div className=''>
							...
							<p className='font-medium tracking-wider text-sm'>{livreurs.find((liv) => liv?.id === client?.id)?.wallets?.find((w) => w?.id === wallet?.id)?.balance} CFA</p>
						</div>
						<div className=''>
							<p className='font-light text-xs'>Bonus</p>
							<p className='font-medium tracking-wider text-sm'>{livreurs.find((liv) => liv?.id === client?.id)?.wallets?.find((w) => w?.id === wallet?.id)?.bonus} CFA</p>
						</div>

						<div className=''>
							<p className='font-light text-xs'>Created On</p>
							<p className='font-bold tracking-more-wider text-sm'>
								{moment(livreurs.find((liv) => liv?.id === client?.id)?.wallets?.find((w) => w?.id === wallet?.id)?.created_at).format('ll')}
							</p>
						</div>
					</div>
				</div>
