...
export const saveMerchantsAccompagnement = createAsyncThunk(
  ...
  async (params) => {
    ...
    const supplementClientPrice = parseInt(params.supplementClientPrice);
    const supplementMerchantPrice = parseInt(params.supplementMerchantPrice);

    console.log(params);

    const payload = JSON.stringify({
      name,
      supplement_client_price: supplementClientPrice,
      supplement_merchant_price: supplementMerchantPrice,
      merchant_id
    });
...
export const saveMerchantsArticle = createAsyncThunk('/merchants/save-article', async (params) => {
  try {
    ...
    formData.append('merchant_price', JSON.stringify(parseInt(params.merchantPrice)));
  ...
  extraReducers: {
    ...
    [saveMerchantsAccompagnement.fulfilled]: (state, action) => {
      ...
      if (indexToReplaced !== -1) {
        state.merchants[indexToReplaced].accompagnements = [
          action.payload?.accompagnement,
          ...state.merchants[indexToReplaced]?.accompagnements
        ];
        state.merchants[indexToReplaced] = state.merchants[indexToReplaced];
      }
...
export const { resetForm } = merchantsSettingsSlice.actions;
