	...
	const columnDefs = useMemo(() => [
		...
		},
		{
			...
			cellRenderer: ({ value }) => {
				return (
					// <div className='flex items-center justify-center'>
					<p className='uppercase break-all overflow-hidden font-semibold'>{value}</p>
					// </div>
				);
			},
		},
		{
			...
			onCellClicked: (params) => openMerchantDetails(params.data),
			cellRenderer: ({ value }) => {
				return <div className='flex items-center justify-center font-semibold'>{value?.length + ' art' || 'N/A'}</div>;
			},
		},
		{
			...
			onCellClicked: (params) => openMerchantDetails(params.data),
			cellRenderer: ({ value }) => {
				return <div className='flex items-center justify-center font-semibold'>{value?.filter((acc) => acc.is_deleted === false)?.length + ' acc' || 'N/A'}</div>;
			},
		},
