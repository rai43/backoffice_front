		...
		<>
			...
			>
				<div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
					...
					<div className='text-primary col-span-2'>{extraObject?.user_type?.libelle?.toLocaleUpperCase()}</div>
					<div className='font-bold'>Creation Date</div>
					<div className='text-primary col-span-2'>{moment(extraObject?.created_at).format('LL')}</div>
					<div className='font-bold'>User Account Status</div>
					<div className='text-primary col-span-2'>
						{extraObject?.is_locked ? <div className='badge badge-sm badge-outline badge-error'>Inactive</div> : <div className='badge badge-outline badge-success'>Active</div>}
					</div>
				</div>
			</TitleCard>
			<TitleCard
				title={`ABOUT THE USER CREATION`}
				topMargin='my-2'
			>
				<div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
					<div className='font-bold'>Creator Last Name</div>
					<div className='text-primary col-span-2'>{extraObject?.UserCreate?.nom?.toLocaleUpperCase()}</div>
					<div className='font-bold'>Creator First Name</div>
					<div className='text-primary col-span-2'>{extraObject?.UserCreate?.prenom?.toLocaleUpperCase()}</div>
