...
const LivreurDetailView = ({ extraObject, closeModal }) => {
	...
	useEffect(() => {
		...
		setClientObj((oldObj) => {
			return { ...oldObj, ...livreurs.find((livreur) => livreur?.id === extraObject?.id) };
		});
	}, []);
	...
	return (
		<div className='w-full'>
			...
			<p className='px-2 text-lg font-semibold uppercase'>{`${livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.last_name} ${
				livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.first_name
			} - (${livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.status})`}</p>
			<div className='w-full grid grid-cols-1 md:grid-cols-4'>
				<div className='col-span-3 p-2 grid grid-rows-2 gap-2'>
						...
						<div>
							<div
								className={`inline-grid rounded-lg w-3 h-3 justify-items-center self-center items-center ${
									livreurs.find((livreur) => livreur?.id === extraObject?.id)?.is_deleted ? 'bg-error' : 'bg-success'
								}`}
							></div>
					...
					{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.second_phone_number?.length && (
						<div>
							<div
								className={`inline-grid rounded-lg w-3 h-3 justify-items-center self-center items-center ${
									livreurs.find((livreur) => livreur?.id === extraObject?.id)?.is_deleted ? 'bg-error' : 'bg-success'
							...
							<div className='inline-grid mx-4 font-semibold'>
								{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country?.prefix
									? livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country.prefix + ' '
									: '+225 '}{' '}
								{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.second_phone_number}
							</div>

							<div className='inline-grid mx-6 font-extralight'>/ second phone number</div>
						</div>
					)}
					{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.whatsapp?.length && (
						<div>
							<div
								className={`inline-grid rounded-lg w-3 h-3 justify-items-center self-center items-center ${
									livreurs.find((livreur) => livreur?.id === extraObject?.id)?.is_deleted ? 'bg-error' : 'bg-success'
					...
					{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.emergency_contact?.length && (
						<div className='text-warning font-semibold'>
							<div className={`inline-grid rounded-lg w-3 h-3 justify-items-center self-center items-center bg-warning`}></div>
							<div className='inline-grid mx-4 font-semibold'>
								{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country?.prefix
									? livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country.prefix + ' '
									: '+225 '}{' '}
								{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.emergency_contact}
							</div>
				...
				<div className='flex flex-row-reverse'>
					<div className='flex-shrink-0 h-14 w-14 mx-5'>
						<img
							...
							src={
								livreurs.find((livreur) => livreur?.id === extraObject?.id)?.photo?.startsWith('http') ? livreurs.find((livreur) => livreur?.id === extraObject?.id)?.photo : streetLogo
							}
							alt='UserImage'
						/>
					</div>
			...
			<div className='w-full stats stats-vertical lg:stats-horizontal shadow'>
				<div className='stat'>
					...
					<div className='stat-value text-[1.5rem]'>{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.livreurs[0]?.id || 'N/A'}</div>
					<div className='stat-desc'>Client ID: {livreurs.find((livreur) => livreur?.id === extraObject?.id)?.id}</div>
				</div>
				...
				<div className='stat'>
					...
					<div className='stat-value text-[1.5rem]'>{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country?.name || 'N/A'}</div>
					<div className='stat-desc'>
						Code: <span className='font-semibold'>{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country?.code || 'N/A'}</span> | Prefix:{' '}
				...
				<div className='stat'>
					...
					<div className='stat-desc'>
						By: <span className='font-semibold'>{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.created_by || 'N/A'}</span>
					</div>
				</div>
			</div>

			<div className='w-full my-5'>
				<h3 className='text-lg font-semibold'>User Card{livreurs.find((livreur) => livreur?.id === extraObject?.id)?.wallets?.length > 1 ? 's' : ''}</h3>
				{/* Divider */}
				<div className='divider mt-0'></div>
									...
									>
										<WalletCardLivreur
											...
											phone_number={
												(livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country?.prefix
													? livreurs.find((livreur) => livreur?.id === extraObject?.id)?.country.prefix + ' '
													: '+225 ') + livreurs.find((livreur) => livreur?.id === extraObject?.id)?.phone_number
											}
