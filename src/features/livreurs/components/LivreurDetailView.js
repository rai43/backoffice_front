...
const LivreurDetailView = ({ extraObject, closeModal }) => {
  ...
  useEffect(() => {
    console.log(
      livreurs.find((livreur) => livreur?.id === extraObject?.id)?.wallets,
    );
    setClientObj((oldObj) => {
      return {
        ...oldObj,
        ...
        <div className="col-span-3 p-2 grid grid-rows-2 gap-2">
          {livreurs.find((livreur) => livreur?.id === extraObject?.id)
            ...
            <div>
              ...
              <div className="inline-grid mx-6 font-extralight">
                / client mobile number
              </div>
            </div>
          )}
          {livreurs.find((livreur) => livreur?.id === extraObject?.id)
            ?.livreurs[0]?.second_phone_number?.length && (
            <div>
              <div
                className={`inline-grid rounded-lg w-3 h-3 justify-items-center self-center items-center ${
                  livreurs.find((livreur) => livreur?.id === extraObject?.id)
              ...
              <div className="inline-grid mx-4 font-semibold">
                ...
                {
                  livreurs.find((livreur) => livreur?.id === extraObject?.id)
                    ?.livreurs[0]?.second_phone_number
                }
              </div>
          ...
          {livreurs.find((livreur) => livreur?.id === extraObject?.id)
            ?.livreurs[0]?.emergency_contact?.length && (
      ...
      <div className="w-full stats stats-vertical lg:stats-horizontal shadow">
        <div className="stat">
          ...
          <div className="stat-desc">
            Client ID:{" "}
            {livreurs.find((livreur) => livreur?.id === extraObject?.id)?.id}
          </div>
        </div>
        ...
        <div className="stat">
          ...
          <div className="stat-desc">
            ...
            | Prefix:{" "}
            <span className="font-semibold">
              {extraObject?.country?.prefix || "N/A"}
            </span>
          </div>
        </div>
        <div className="stat">
              ...
              {livreurs
                ...
                ?.wallets?.map((wallet) => (
                  <div key={wallet.id} className="p-3">
                    <WalletCardLivreur
                      client={livreurs.find(
                        (livreur) => livreur?.id === extraObject?.id,
                      )}
                      wallet={wallet}
                      phone_number={
                        (livreurs.find(
