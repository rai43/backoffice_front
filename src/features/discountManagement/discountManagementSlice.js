...
export const getDiscounts = createAsyncThunk('/discount-management/get-discounts', async (params) => {
	const from = params.from || moment().subtract(7, 'days').format('YYYY-MM-DD');
	const to = params.to || moment().add(1, 'days').format('YYYY-MM-DD');
...
export const saveDiscounts = createAsyncThunk('/discount-management/save-discount', async (params) => {
	const fromDisplay = params.fromDisplay || moment().subtract(7, 'days').toDate();
	const toDisplay = params.toDisplay || moment().toDate();
	...
	const payload = JSON.stringify({ from, to, fromDisplay, toDisplay, discount_type, value, description, articles });

	const response = await axios.post('/api/discounts/save-dicounts', payload);
	return response.data;
});

export const updateDiscount = createAsyncThunk('/discount-management/update-discount-by-id', async (params) => {
	const from = moment(params.from).toDate();
	const to = moment(params.to).toDate();
	const discount_type = params.type;
	const value = params.value;
	const description = params.description;
	const discount = params.discount;

	const payload = JSON.stringify({ from, to, discount_type, value, description });

	const response = await axios.post(`/api/discounts/update-dicount/${discount}`, payload);
	return response.data;
});

export const toggleDiscountStatus = createAsyncThunk('/discount-management/toggle-discount-status', async (params) => {
...
export const deleteDiscount = createAsyncThunk('/discount-management/delete-discount-by-id', async (params) => {
	const discountId = params.discountId;
...
export const discountManagementSlice = createSlice({
	...
	extraReducers: {
		...
		[updateDiscount.fulfilled]: (state, action) => {
			const indexToRemoved = state.discounts.findIndex((discount) => discount.id === action.payload?.discount?.id);
