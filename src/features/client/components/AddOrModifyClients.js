...
const AddOrModifyClients = ({ extraObject }) => {
	...
	const personalFormik = useFormik({
		initialValues: client?.phone_number ? { ...INITIAL_PERSONAL_OBJ, phone_number: '+225' + client.phone_number } : INITIAL_PERSONAL_OBJ,
		...
		<div className='w-full '>
			<div className='flex items-center justify-center'>
				<ul className='steps'>
					...
					{selectedAccount === 'personal' ? <li className={`step ${activeStep >= 2 ? 'step-primary' : ''}`}>Summary</li> : <></>}
					{selectedAccount === 'merchant' ? <li className={`step ${activeStep >= 2 ? 'step-primary' : ''}`}>Locations</li> : <></>}
					{selectedAccount === 'merchant' ? <li className={`step ${activeStep >= 3 ? 'step-primary' : ''}`}>Work Schedule</li> : <></>}
					{selectedAccount === 'merchant' ? <li className={`step ${activeStep >= 4 ? 'step-primary' : ''}`}>Summary</li> : <></>}
				</ul>
			</div>
