...
import { AG_GRID_DEFAULT_COL_DEF, CONFIRMATION_MODAL_CLOSE_TYPES, MODAL_BODY_TYPES } from '../../../utils/globalConstantUtil';
		...
		() => [
			{
				// field: 'phone_number',
				valueGetter: ({ data }) => {
					return data ? data?.phone_number + (data?.merchants[0] ? data?.merchants[0]?.name : '') : ''; // Adjust this based on your actual structure
				},
				headerName: 'Client',
				width: 260,
				// flex: 3,
			...
			},
			{
				// field: 'created_at',
				valueGetter: (params) => {
					return params?.data?.created_at ? moment(moment(params?.data?.created_at).format('DD/MM/YYYY')).toDate() : 'N/A'; // Adjust this based on your actual structure
				},
				...
				cellRenderer: ({ value, data }) => {
					...
					if (formattedValue !== 'N/A') {
						formattedValue = moment(data?.created_at).format('DD/MM/YYYY HH:mm');
					}
