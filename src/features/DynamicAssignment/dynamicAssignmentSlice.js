...
export const getNotAssignedOrders = createAsyncThunk('/orders/get-all-not-assigned-orders', async (params) => {
...
export const markPresentLivreursPerZone = createAsyncThunk('/dynamic/mark-present-livreurs-per-zone', async (params) => {
	const zone = params.zone;
	const livreurs = params.livreurs;

	const payload = JSON.stringify({ zone, livreurs });

	const response = await axios.post(`/api/dynamic-assignment/mark-present-livreurs-per-zone`, payload);
...
export const dynamicAssignment = createAsyncThunk('/dynamic/assign-livreurs-to-orders', async (params) => {
