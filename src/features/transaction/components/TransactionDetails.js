...
const WalletCard = ({ wallet }) => {
	return (
		<div className={`w-full border-base-100 rounded-lg shadow-[0_3px_10px_rgb(0,0,0,0.2)] max-h-[15rem] mt-3 p-4`}>
			<div className='flex flex-col items-center pb-4'>
				...
				<h5 className='mb-1 text-xl font-medium text-gray-900 dark:text-white'>{wallet?.client?.phone_number}</h5>
		...
		<>
			<div className='stats shadow w-full'>
				...
				<div className='stat place-items-center'>
					...
					<div className='stat-value text-secondaryd text-[1.8rem]'>{extraObject?.amount || 0} FCFA</div>
					<div className={`stat-desc font-semibold ${extraObject?.transaction_status?.code === 'COMPLETED' ? 'text-success' : ''}`}>{extraObject?.transaction_status?.code || 'N/A'}</div>
				</div>

				<div className='stat place-items-center'>
					<div className='stat-title'>Date</div>
					<div className='stat-value text-[1.3rem]'>{moment(extraObject?.created_at).format('LL')}</div>
					<div className='stat-desc'>{moment(extraObject?.created_at).format('LT')}</div>
				</div>
				<div className='stat place-items-center'>
					<div className='stat-title'>Date</div>
					<div className='stat-value text-[1.3rem]'>{moment(extraObject?.created_at).format('LL')}</div>
			...
			<div className='grid grid-cols-1 sm:grid-cols-3 gap-3 p-5'>
				<div className='w-full'>
					<WalletCard wallet={extraObject?.sender_wallet} />
				</div>
				...
				<div className='w-full'>
					<WalletCard wallet={extraObject?.receiver_wallet} />
				</div>
			</div>
		</>
	);
