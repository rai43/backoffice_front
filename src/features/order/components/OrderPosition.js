...
const OrderPosition = ({ extraObject }) => {
	...
	const [position, setPosition] = useState(
		extraObject?.all
			...
			? [extraObject?.orders[0]?.address?.latitude || 0, extraObject?.orders[0]?.address?.longitude || 0]
			...
			>
				...
				{!extraObject?.all && !extraObject?.selectedRows && (
					...
					>
						<Popup>
							Client <br />
							<span className='text-primary font-bold'>{extraObject?.order?.client?.phone_number}</span>
