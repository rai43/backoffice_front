		...
		<div>
			...
			<div className='grid md:grid-cols-2 gap-3 uppercase'>
				...
				<div className='grid grid-cols-3 font-semibold'>
					...
					<div className='col-span-2 text-primary'>
						...
						{parseInt(extraObject?.total_discount) > 0 ? <span className='text-secondary mx-2'>(- {extraObject?.total_discount})</span> : ''}
					</div>
				</div>
				<div className='grid grid-cols-3 font-semibold'>
			...
			<div className='grid md:grid-cols-2 gap-3 uppercase'>
				...
				<div className='grid grid-cols-3 font-semibold'>
					...
					<div className='col-span-2 text-primary'>{extraObject?.merchant?.client?.phone_number}</div>
				</div>
			</div>
				...
				<div className='stat place-items-center'>
					...
					<div className='stat-value'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'PENDING')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'PENDING')?.created_at).format('HH:mm')
							: '- -'}
					</div>
					<div className='stat-desc'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'PENDING')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'PENDING')?.created_at).format('DD-MM-YYYY')
							: '- -'}
					</div>
				</div>
				...
				<div className='stat place-items-center'>
					...
					<div className='stat-value text-'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'REGISTERED')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'REGISTERED')?.created_at).format('HH:mm')
							: '- -'}
					</div>
					<div className='stat-desc text-'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'REGISTERED')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'REGISTERED')?.created_at).format('DD-MM-YYYY')
							: '- -'}
					</div>
				</div>
				...
				<div className='stat place-items-center'>
					...
					<div className='stat-value text-'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INPROCESS')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INPROCESS')?.created_at).format('HH:mm')
							: '- -'}
					</div>
					<div className='stat-desc text-'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INPROCESS')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INPROCESS')?.created_at).format('DD-MM-YYYY')
							: '- -'}
					</div>
				</div>
				...
				<div className='stat place-items-center'>
					...
					<div className='stat-value text-'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INDELIVERY')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INDELIVERY')?.created_at).format('HH:mm')
							: '- -'}
					</div>
					<div className='stat-desc text-'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INDELIVERY')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'INDELIVERY')?.created_at).format('DD-MM-YYYY')
							: '- -'}
					</div>
				</div>
				...
				<div className='stat place-items-center'>
					...
					<div className='stat-value text-primary'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'DELIVERED')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'DELIVERED')?.created_at).format('HH:mm')
							: '- -'}
					</div>
					<div className='stat-desc text-primary'>
						{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'DELIVERED')
							? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'DELIVERED')?.created_at).format('DD-MM-YYYY')
							: '- -'}
					</div>
				</div>
				{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'CANCELED') && (
					<div className='stat place-items-center'>
						...
						<div className='stat-value text-secondary'>
							{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'CANCELED')
								? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'CANCELED')?.created_at).format('HH:mm')
								: '- -'}
						</div>
						<div className='stat-desc text-secondary'>
							{extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'CANCELED')
								? moment(extraObject?.commande_commande_statuses.find((status) => status?.commande_status?.code === 'CANCELED')?.created_at).format('DD-MM-YYYY')
								: '- -'}
						</div>
					</div>
				)}
			...
			<div className='my-4 flex justify-center'>
				<select
					...
					onChange={async (e) => {
						...
						await dispatch(setOrderStatusNoCheck({ commandId: extraObject?.id, status: e.target.value })).then(async (response) => {
							if (response?.error) {
								dispatch(
									showNotification({
					...
					<div key={article?.id}>
						...
						<div className='grid md:grid-cols-3 gap-2'>
							...
							<div className='md:col-span-2 grid grid-cols-3 uppercase font-semibold'>
								...
								{article?.supplements?.length > 0 ? <div className='col-span-3 divider text-secondary'>Supplements</div> : ''}
