...
const ClientOrdres = ({ currPage, onLoad, updateFormValue, client, dateValues }) => {
	...
	const onChangeLivreur = (data) => {
		...
		if (COMMANDE_STATUS_CODE_VS_NUMBERS[status] > 1 && COMMANDE_STATUS_CODE_VS_NUMBERS[status] < 5) {
			dispatch(
				openModal({
	...
	const columnDefs = useMemo(() => [
		...
		},
		{
			...
			valueGetter: ({ data }) => {
				...
				return livreur?.last_name + ' ' + livreur?.first_name + livreur?.client?.phone_number; // Adjust this based on your actual structure
			},
			filterParams: containFilterParams,
			...
			cellRenderer: ({ value, data }) => {
				return (
					<div className='grid grid-row-2 text-xs font-semibold hover:cursor-pointer'>
						<p className={classNames('p-1 uppercase leading-wide')}>{`${data?.livreur?.last_name ? data?.livreur?.last_name : ''} ${
							data?.livreur?.first_name ? data?.livreur?.first_name : ''
						}`}</p>
						<p className={classNames('p-1 uppercase leading-wide')}>{`${data?.livreur?.client?.phone_number ? data?.livreur?.client?.phone_number : ''}`}</p>
					</div>
				);
			},
		},
		...
		},
		{
			...
			cellRenderer: ({ value }) => {
				...
				return <button className='btn btn-outline btn-xs btn-primary'> {COMMANDE_NUMBERS_VS_STATUS_CODE[(COMMANDE_STATUS_CODE_VS_NUMBERS[value] || 0) + 1]}</button>;
	...
	const onButtonClick = () => {
		...
		link.setAttribute('download', `export_data_from_${dateValues?.startDate}_to_${dateValues?.endDate}.csv`);
			...
			<div className='overflow-hidden mt-3'>
				{!isLoading && (
					<>
									...
									<button
										...
										onClick={() => {
											...
											dispatch(
												openModal({
													title: 'Positions',
													extraObject: { orders: selectedRows, selectedRows: true },
													bodyType: MODAL_BODY_TYPES.ORDER_POSITION,
													size: 'lg',
												})
											);
							...
							<div className='font-semibold'>
								...
								<select
									...
									onChange={(e) => {
										...
										console.log('gridOptions.api.paginationPageSize', gridOptions.api.paginationProxy.pageSize);
									}}
									defaultValue={20}
								>
						...
						<div className='ag-theme-alpine hidden md:block'>
							<AgGridReact
								ref={gridRef}
