...
export const getColis = createAsyncThunk('/parcels-management/get-colis', async (params) => {
  ...
  const colisInfo = params.colisInfo;
...
export const payColisMerchantBulk = createAsyncThunk(
  '/parcels-management/pay-colis-merchant-bulk',
  async (params, { rejectWithValue }) => {
...
export const parcelsManagementSlice = createSlice({
  ...
  extraReducers: {
    ...
    [updateColis.fulfilled]: (state, action) => {
      const indexToReplace = state.colis.findIndex((c) => c.id === action.payload?.colis?.[0]?.id);
      console.log({
        indexToReplace,
        action: action.payload?.colis?.[0]?.id,
        colis: action.payload?.colis?.[0],
        state: state.colis
      });

      if (indexToReplace !== -1) {
        state.colis[indexToReplace] = action.payload?.colis?.[0];
